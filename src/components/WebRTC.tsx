                        ...
                        const applySavedDevice = async () => {
                          ...
                          if (targetDeviceId && targetDeviceId !== 'default') {
                            try {
                              ...
                              const deviceExists = audioOutputDevices.some(
                                (device) => device.deviceId === targetDeviceId,
                              )

                              if (!deviceExists) {
                                console.warn(
                                  `Saved audio device ${targetDeviceId} no longer available, using default device`,
                                )
                                targetDeviceId = 'default'
                                // Update localStorage with the fallback
                                setJSONItem('phone-island-audio-output-device', {
                                  deviceId: 'default',
                                })
                              }
                            } catch (err) {
                              console.warn(
                                'Error checking device availability, using default:',
                                err,
                              )
                              targetDeviceId = 'default'
                            }
                          ...
                          try {
                            ...
                            console.info(
                              'Audio output device applied successfully to new stream:',
                              targetDeviceId,
                            )
                          } catch (err) {
