  ...
  useEffect(() => {
    if (reload || connectionReturned) {
      ...
      if (isWebRTCDown || forceReload) {
        ...
        if (isReloading.current) {
          console.log('[JANUS-GUARD] Reload already in progress, skipping')
          return
        }
        ...
        setTimeout(() => {
          ...
          // Reset reload flag after completion
          setTimeout(() => {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      ...
      if (document.hidden) {
        ...
        const shouldReload = registered && !isReloading.current && needsReload
